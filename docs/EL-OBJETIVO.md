...
1. **Nada celebra terminar una sesión.** Es lo que más falta para la sensación
   de libreta. Marcas el check y no pasa nada.
2. **Copia de seguridad automática.** Todo vive en el `localStorage` de un
   navegador. Hoy hay exportación manual, que depende de acordarse. Ver
   `docs/MEJORAS.md` (M1).
3. **Registrar actividad no planificada** (una pachanga, una caminata larga) y
   eventos de agenda sueltos.
4. **Reglas de ajuste automático** cuando se falla un día: hoy se mueve la
   sesión a mano.
